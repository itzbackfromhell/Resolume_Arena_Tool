...
class AlphaDropperApp(tk.Tk):
    """Desktop GUI with presets, input preview, processed preview, and batch export."""
    ...
    def _build_mode_panel(self, parent: ttk.Frame) -> None:
        ...
        ttk.Radiobutton(panel, text="Single image", variable=self.mode_var, value="single", command=self._refresh_preview).pack(side=tk.LEFT)
        ttk.Radiobutton(panel, text="Batch folder", variable=self.mode_var, value="batch", command=self._refresh_preview).pack(side=tk.LEFT, padx=(12, 0))
    ...
    def _build_path_panel(self, parent: ttk.Frame) -> None:
        ...
        ttk.Entry(panel, textvariable=self.input_var, width=44).grid(row=0, column=1, sticky="ew", pady=4)
        ttk.Button(panel, text="Browse", command=self._browse_input).grid(row=0, column=2, padx=(8, 0))
        ttk.Label(panel, text="Output folder").grid(row=1, column=0, sticky="w", padx=(0, 8), pady=4)
        ttk.Entry(panel, textvariable=self.output_var, width=44).grid(row=1, column=1, sticky="ew", pady=4)
        ttk.Button(panel, text="Browse", command=self._browse_output).grid(row=1, column=2, padx=(8, 0))
        ttk.Label(panel, text="Paste paths directly or use Browse.", foreground="#555555").grid(row=2, column=0, columnspan=3, sticky="w", pady=(4, 0))
    ...
    def _build_action_panel(self, parent: ttk.Frame) -> None:
        ...
        ttk.Button(panel, text="Clear log", command=self._clear_log).grid(row=1, column=0, columnspan=2, sticky="ew", pady=3)
    ...
    def _bind_preview_traces(self) -> None:
        self.input_var.trace_add("write", lambda *_: self._refresh_preview())
    ...
    def _browse_input(self) -> None:
        selected = filedialog.askopenfilename(title="Select image", filetypes=IMAGE_FILETYPES) if self.mode_var.get() == "single" else filedialog.askdirectory(title="Select input folder")
        if selected:
    ...
    def _preview_source(self) -> Path | None:
        ...
        if path.is_dir():
            for candidate in sorted(path.rglob("*")):
                if candidate.suffix.lower() in {".png", ".jpg", ".jpeg", ".webp", ".bmp", ".tif", ".tiff"}:
                    return candidate
        return None
    ...
    def _start_preview(self, token: int) -> None:
        ...
        if not source or (self.preview_worker and self.preview_worker.is_alive()):
            return
        try:
        ...
        self.preview_worker = threading.Thread(target=self._preview_worker_fn, args=(source, token), daemon=True)
        self.preview_worker.start()

    def _preview_worker_fn(self, source: Path, token: int) -> None:
        try:
            with Image.open(source) as image:
                processed = process_image_object(image, self._options())
    ...
    def _process_worker(self) -> None:
        try:
            ...
            output_dir = Path(self.output_var.get().strip().strip('"'))
            output_dir.mkdir(parents=True, exist_ok=True)
            if self.mode_var.get() == "single":
                output_path = build_output_path(input_path, output_dir, suffix=options.normalized_suffix(), extension=options.output_format, overwrite=options.overwrite)
                result = process_single(input_path, output_path, options, on_progress=self.log_queue.put)
                self.log_queue.put(f"DONE {result.output_path} ({result.width}x{result.height})")
            else:
                summary = process_directory(input_path, output_dir, options, on_progress=self.log_queue.put)
